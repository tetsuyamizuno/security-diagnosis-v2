...
- HTTPヘッダー未設定・情報開示による追加減点（該当するものを合算）：
  ...
  - X-Frame-Options 未設定：−3点（クリックジャッキングによる不正操作・認証情報詐取を許容するため）
  - X-Content-Type-Options 未設定：−3点（MIMEスニッフィングによる悪意あるコード実行を許容するため）
  - Referrer-Policy 未設定：−3点（参照元URLが外部サイトに漏洩し、内部パス等が露出するため）
  - Permissions-Policy 未設定：−2点（カメラ・マイク・位置情報等の機能を第三者が悪用できるため）
