...
- HTTPヘッダー未設定・情報開示による追加減点（該当するものを合算）：
  - CSP 未設定：−5点（XSS・スクリプトインジェクションを直接許容する最重大リスク）
  - X-Frame-Options 未設定：−5点（クリックジャッキングによる不正操作・認証情報詐取を許容するため）
  - X-Content-Type-Options 未設定：−5点（MIMEスニッフィングによる悪意あるコード実行を許容するため）
  - Permissions-Policy 未設定：−3点（カメラ・マイク・位置情報等の機能を第三者が悪用できるため）
  - Referrer-Policy 未設定：−3点（参照元URLが外部サイトに漏洩し、内部パス等が露出するため）
  - X-Powered-By 情報開示：−1点（サーバー技術情報が攻撃者に伝わり、標的型攻撃を助けるため）
  - Server情報開示：−1点（Webサーバー種別・バージョンが攻撃者に伝わるため）
- スコアの下限は0点
