...
OWASP Top 10 2025（A01:アクセス制御の不備 / A02:セキュリティ設定ミス / A03:ソフトウェアサプライチェーンの障害［新規］ / A04:暗号化の失敗 / A05:インジェクション / A06:安全でない設計 / A07:認証の失敗 / A08:ソフトウェア・データ整合性の失敗 / A09:セキュリティロギングの失敗 / A10:例外状態の不適切な処理［新規］）/NVD・CVE公式情報/JPCERT/CC注意喚起/IPA情報セキュリティ10大脅威最新版/警察庁サイバー犯罪統計/フィッシング対策協議会月次報告/IBM Cost of a Data Breach最新版/Verizon DBIR最新版/CISA Advisory/ベンダー公式セキュリティアドバイザリ
...
【重要】<footer>タグを含むフッター要素をHTMLに一切出力しないこと。著作権表記・サイト名・バージョン情報等も不要。
...
2. 【必須】総合リスクスコア（100点満点）← 冒頭注記の直後に必ず表示すること
   - プログレスバー形式でスコアを大きく表示（例：72 / 100 点）
   - スコアに対応するレベルラベル（軽微／中程度／深刻／非常に危険）
   - 一言コメント（経営者向け・平易な言葉）
   - ※ 〇〇点満点中△△点。高リスク×件、中リスク×件、低リスク×件が検出されています。
   - セキュリティヘッダー チェック結果表（列：ヘッダー名／状況（対応済み or 未対応）／リスク内容）
3. 診断範囲・取得状況（表）
4. 対象サマリー（表）
5. HTML・公開リソース分析結果（表）
6. HTTPヘッダー分析結果（表）
...
7. <footer>タグ・フッター要素・著作権表記をHTMLに一切出力しないこと

━━━ 総合リスクスコア（100点満点）━━━
冒頭注記セクションのHTML直後に、以下のHTMLブロックを必ず挿入してください。
[SCORE]・[LEVEL]・[COMMENT]・[BREAKDOWN]・[HEADER_TABLE] は計算結果で置き換えること。

<section class="score-section" style="background:#fff;border-radius:10px;padding:32px 40px;margin:32px auto;max-width:960px;box-shadow:0 2px 12px rgba(0,0,0,.07);">
  <h2 style="font-family:'Noto Serif JP',serif;font-size:1.2rem;color:#1E2A35;margin:0 0 16px;">セキュリティ健全性スコア</h2>
  <p style="color:#1E2A35;margin:0 0 20px;">[COMMENT]</p>
  <div style="background:#e8e8e8;border-radius:999px;height:36px;overflow:hidden;margin-bottom:12px;">
    <div style="width:[SCORE]%;background:#4CAF82;height:100%;border-radius:999px;display:flex;align-items:center;justify-content:flex-end;padding-right:16px;">
      <span style="color:#fff;font-weight:700;font-size:1.1rem;">[SCORE] / 100 点</span>
    </div>
  </div>
  <p style="color:#2D6A8F;font-size:0.9rem;margin:0 0 24px;">※ 100点満点中[SCORE]点。[BREAKDOWN]</p>
  <h3 style="font-size:1rem;color:#1E2A35;margin:0 0 12px;">セキュリティヘッダー チェック結果</h3>
  [HEADER_TABLE]
</section>

【算出ルール】基準点：100点
- 高リスク1件ごとに −15点 / 中リスク1件ごとに −7点 / 低リスク1件ごとに −2点
- ヘッダー追加減点：CSP未設定−5点・X-Frame-Options未設定−3点・X-Content-Type-Options未設定−3点・Referrer-Policy未設定−3点・Permissions-Policy未設定−2点・X-Powered-By情報開示−1点・Server情報開示−1点
- スコア下限は0点

[LEVEL]：80〜100点=軽微なリスク / 60〜79点=中程度のリスク / 40〜59点=深刻なリスク / 0〜39点=非常に危険

[HEADER_TABLE]：ヘッダー名・状況（対応済み/未対応）・リスク内容の3列テーブル。減点数値は記載しない。
`.trim();
...
【重要】スコアセクションの直後に、以下の形式でヘッダーチェック結果表を必ず出力すること：
- 表のタイトル：「セキュリティヘッダー チェック結果」
- 列：ヘッダー名 ／ 状況（対応済み or 未対応）／ リスク内容
- 「リスク内容」は対応済み・未対応にかかわらず全行に記載すること
  - 未対応の場合：そのヘッダーがないことで生じるリスクを記載
  - 対応済みの場合：そのヘッダーが設定されていることで防いでいるリスクを記載
- 減点の数値は表に記載しないこと
- スコアの下限は0点
...
④ 気になった点は自律的に追加調査
⑤ OWASP Top 10 2025・NVD・JPCERT/CC・IPA・IBM等の公的統計で事例と損害額を裏取り
