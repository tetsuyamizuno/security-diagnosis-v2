...
━━━ 総合リスクスコア（100点満点）━━━
冒頭注記（セクション1）の直後に、必ずスコアセクションをHTML出力してください。

【算出ルール】基準点：100点
- 高リスク1件ごとに −15点
- 中リスク1件ごとに −7点
- 低リスク1件ごとに −2点
- HTTPヘッダー未設定・情報開示による追加減点：
...
【出力形式】
- プログレスバーでスコアを大きく表示（例：72 / 100 点）
- レベルラベル（軽微／中程度／深刻／非常に危険）
- ※ 100点満点中○○点。高リスク×件、中リスク×件、低リスク×件が検出されています。
- 一言コメント（経営者向け・平易な言葉）
- セキュリティヘッダー チェック結果表（列：ヘッダー名／状況（対応済み or 未対応）／リスク内容）
  - リスク内容は対応済み・未対応いずれも全行に記載すること
  - 減点の数値は表に記載しないこと
`.trim();
