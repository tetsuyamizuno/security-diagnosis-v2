...
- HTTPヘッダー未設定・情報開示による追加減点（該当するものを合算）：
  - X-Frame-Options 未設定：−5点（クリックジャッキング攻撃を許容するため）
  - X-Content-Type-Options 未設定：−5点（MIMEスニッフィング攻撃を許容するため）
  - CSP 未設定：−5点（XSS・スクリプトインジェクションを許容するため）
  - Referrer-Policy 未設定：−3点（参照元URLが外部サイトに漏洩するため）
  - Permissions-Policy 未設定：−3点（カメラ・位置情報等の機能制限がないため）
  - X-Powered-By 情報開示：−1点（サーバー技術情報が攻撃者に伝わるため）
  - Server情報開示：−1点（Webサーバー種別が攻撃者に伝わるため）
- スコアの下限は0点
